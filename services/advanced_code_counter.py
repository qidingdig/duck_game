...
class AdvancedCodeCounter:
    ...
    # C/C++文件分类说明：
    # 1. ".cpp" -> "C++": C++源文件（实现文件）
    # 2. ".hpp" -> "C++ Header": C++专用头文件，明确标识为C++代码
    # 3. ".h" -> "C/C++ Header": C/C++通用头文件，可能是：
    #    - C语言头文件（如 stdio.h, stdlib.h）
    #    - C++兼容C的头文件（如 string.h, math.h）
    #    - C++项目中使用的C风格头文件
    # 
    # 这样分类的考量：
    # - 区分C++专用头文件(.hpp)和通用头文件(.h)，便于分析项目架构
    # - .h文件可能是C或C++，分开统计有助于了解代码库的语言组成
    # - 在实际项目中，.hpp通常表示纯C++代码，而.h可能是跨语言兼容的
    # - 这种分类有助于代码审查和重构时的决策
    EXT_TO_LANGUAGE = {
        ".py": "Python",
        ".pyw": "Python",
